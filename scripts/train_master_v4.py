...
LOG_FILE_PATH = ROOT_DIR / "training_log_master_v7.txt"
GPU_LOG_FILE_PATH = ROOT_DIR / "gpu_profiling_log.csv" # (NEW)
LAST_CKPT_PATH = MODEL_SAVE_PATH / "last_checkpoint.pth"

# --- (NEW) Setup Main Logger ---
# This is now explicitly named 'main_logger'
logging.basicConfig(
    level=logging.INFO,
...
# --- (NEW) Setup GPU Logger ---
# A separate logger that *only* writes to the CSV
gpu_logger = logging.getLogger('gpu_profiling')
gpu_logger.setLevel(logging.INFO)
# Don't add a formatter, we just want the raw message (our CSV row)
gpu_handler = logging.FileHandler(GPU_LOG_FILE_PATH)
gpu_logger.addHandler(gpu_handler)
# Prevent gpu_logger messages from bubbling up to the main logger
gpu_logger.propagate = False 

# --- GPU Monitor Thread ---
class GpuMonitor(threading.Thread):
    def __init__(self, stop_event, device_index=0):
        super().__init__()
        ...
        try:
            ...
            main_logger.warning(f"Could not initialize GPU monitor: {e}")
            self.handle = None
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self.samples.append({"util": util, "temp": temp})
            except Exception:
                pass 
            time.sleep(1)
    ...
    def get_averages(self):
        if not self.samples:
            return 0, 0
        avg_util = np.mean([s['util'] for s in self.samples])
        avg_temp = np.mean([s['temp'] for s in self.samples])
        return avg_util, avg_temp
...
def main(args):
    ...
    device_index = args.device_index if "cuda" in device else 0
    
    amp_enabled = (device != "cpu")
    amp_dtype = torch.bfloat16 if torch.cuda.is_available() and torch.cuda.is_bf16_supported() else torch.float16
    
    main_logger.info(f"--- DeepSight Master v7 Training Run ---")
    ...
    if "cuda" in device:
        try:
            pynvml.nvmlInit()
            main_logger.info("NVML Initialized for GPU monitoring.")
            gpu_monitoring_enabled = True
        except Exception as e:
            main_logger.error(f"Failed to initialize NVML: {e}. GPU monitoring will be disabled.")
    ...
    if args.resume:
        ...
        # (NEW) Write CSV header only on a fresh run
        gpu_logger.info("epoch,avg_util_pct,avg_temp_c,peak_vram_gb")
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            if gpu_monitoring_enabled:
                monitor_thread = GpuMonitor(stop_event, device_index)
            ...
            if monitor_thread:
                stop_event.set()
                monitor_thread.join()
            avg_util, avg_temp = (monitor_thread.get_averages() if monitor_thread else (0, 0))
            ...
            # (NEW) Log GPU stats to separate CSV file
            gpu_log_message = f"{epoch+1},{avg_util:.1f},{avg_temp:.1f},{max_vram_gb:.2f}"
            gpu_logger.info(gpu_log_message)
        ...
        if gpu_monitoring_enabled:
            pynvml.nvmlShutdown()
            main_logger.info("NVML Shutdown complete.")

    main_logger.info(f"\nTraining complete! Best validation loss: {best_val_loss:.4f} ✨")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DeepSight Master Training Script v7")
